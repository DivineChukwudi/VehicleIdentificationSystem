package com.vis.view; 
 ...
 public class SplashScreen extends Application { 
     ...
     // Detect if running inside jPro (no real display/audio available) 
     private static final boolean IS_JPRO = 
             System.getProperty("jpro.running", "false").equalsIgnoreCase("true") 
             || System.getenv("JPRO_RUNNING") != null; 
     ...
     public void start(Stage stage) { 
         ...
         // Only attempt sound when NOT running in jPro (i.e. local desktop) 
         flash.setOnFinished(e -> { if (!IS_JPRO) playTypingSound(); }); 
         ...
         // Only play startup sound on desktop 
         if (!IS_JPRO) playStartupSound(); 
         sequence.play(); 
     } 
     ...
     private void playStartupSound() { 
         try { 
             String path = Objects.requireNonNull( 
                     getClass().getResource("/com/vis/media/startup.mp3")).toExternalForm(); 
     ...
     private void playTypingSound() { 
         try { 
             String path = Objects.requireNonNull( 
                     getClass().getResource("/com/vis/media/typing.mp3")).toExternalForm(); 
             javafx.scene.media.Media sound = new javafx.scene.media.Media(path); 
             javafx.scene.media.MediaPlayer mediaPlayer = new javafx.scene.media.MediaPlayer(sound); 
             mediaPlayer.play(); 
         } catch (Exception e) { 
